...
--	AND s_w_id = :ol_supply_w_id;*/
  ...
  --table.insert(status, string.format("'stock%d', 's_quantity', '{\"s_i_id\":%d, \"s_w_id\": %d}'", table_num, ol_i_id, ol_supply_w_id))
...
--  WHERE w_id =:w_id
  ...
  table.insert(status, string.format("'warehouse%d', 'w_ytd', '{\"w_id\":%d}', 'w_id = %d'", table_num, w_id, w_id))
...
--		AND d_id = :d_id;*/
  ...
  table.insert(status, string.format("'district%d', 'd_ytd', '{\"d_w_id\":%d, \"d_id\": %d}', 'd_w_id = %d AND d_id = %d'", table_num, w_id, d_id, w_id, d_id))
  con:query(([[UPDATE district%d 
                 SET d_ytd = d_ytd + %d 
               WHERE d_w_id = %d 
