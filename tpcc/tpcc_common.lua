...
function exec_tx(command)
   con:query(command)
end


function add_status(commit)
   for _, stat in ipairs(status) do
      if batched_status[stat] == nil then
         batched_status[stat] = {}
         batched_status[stat]["true"] = 0
         batched_status[stat]["false"] = 0
      end
      batched_status[stat][commit] = batched_status[stat][commit] + 1
   end

   status = {}
   current_tx_index = current_tx_index + 1

   if current_tx_index == num_tx_to_insert_status then
      for stat, v in pairs(batched_status) do
         while not pcall(exec_tx, string.format("BEGIN; INSERT INTO tx_status (table_name, column_name, pk, pk_sql, commits, aborts, last_updated) VALUES (%s,%s,%s,now()) ON CONFLICT (table_name, column_name, pk) DO UPDATE SET commits = tx_status.commits + %s, aborts = tx_status.aborts + %s, last_updated = now(); COMMIT", 
            stat, v["true"], v["false"], v["true"], v["false"])
         ) do
         end
         batched_status[stat]["true"] = 0
         batched_status[stat]["false"] = 0
...
function add_status_dummy(commit)
   status = {}
end
