...
public:
	...
	table_t * 		t_warehouse;
	table_t * 		t_warehouse_w_ytd;
	table_t * 		t_district;
	table_t * 		t_district_d_ytd;
	table_t * 		t_customer;
	table_t *		t_history;
	table_t *		t_neworder;
	table_t *		t_order;
	...
	INDEX * 	i_item;
	INDEX * 	i_warehouse;
	INDEX * 	i_warehouse_w_ytd;
	INDEX * 	i_district;
	INDEX * 	i_district_d_ytd;
	INDEX * 	i_customer_id;
...
private:
	...
	void init_tab_warehouse_w_ytd(uint32_t wid, double w_ytd);
	void init_tab_dist(uint64_t w_id);
	void init_tab_district_d_ytd(uint64_t wid, uint64_t did, double d_ytd);
...
private:
	...
	RC run_simple_update(tpcc_query *query);
	RC run_simple_read(tpcc_query *query);
};
