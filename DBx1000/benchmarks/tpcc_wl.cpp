...
RC tpcc_wl::init_schema(const char * schema_file) {
	...
	t_warehouse_w_ytd = tables["WAREHOUSE_W_YTD"];
	t_district = tables["DISTRICT"];
	t_district_d_ytd = tables["DISTRICT_D_YTD"];
	...
	i_warehouse_w_ytd = indexes["WAREHOUSE_W_YTD_IDX"];
	i_district = indexes["DISTRICT_IDX"];
	i_district_d_ytd = indexes["DISTRICT_D_YTD_IDX"];
...
void tpcc_wl::init_tab_wh(uint32_t wid) {
	...
	//mrv
	if (TYPE == MRV) {
		this->init_tab_warehouse_w_ytd(wid, w_ytd);
	}
...
void tpcc_wl::init_tab_warehouse_w_ytd(uint32_t wid, double w_ytd) {
	double partitioned_amount = w_ytd / MRV_SIZE;
	for (int rk = 0; rk < MRV_SIZE; rk++) {
		row_t *row;
		uint64_t row_id;
		this->t_warehouse_w_ytd->get_new_row(row, 0, row_id);
		uint64_t key = (wid << 8) | rk;
		row->set_primary_key(key);
		row->set_value(0, key);
		row->set_value(1, partitioned_amount);
		index_insert(i_warehouse_w_ytd, key, row, wh_to_part(wid));
	}
}
...
void tpcc_wl::init_tab_dist(uint64_t wid) {
	for (uint64_t did = 1; did <= DIST_PER_WARE; did++) {
		...
		//mrv
		if (TYPE == MRV) {
			this->init_tab_district_d_ytd(wid, did, w_ytd);
		}
	}
}


void tpcc_wl::init_tab_district_d_ytd(uint64_t wid, uint64_t did, double d_ytd) {
	double partitioned_amount = d_ytd / MRV_SIZE;
	for (int rk = 0; rk < MRV_SIZE; rk++) {
		row_t* row;
		uint64_t row_id;
		this->t_district_d_ytd->get_new_row(row, 0, row_id);
		uint64_t key = wid << 16 | did << 8 | rk;
		row->set_primary_key(key);
		row->set_value(0, key);
		row->set_value(1, partitioned_amount);
		index_insert(i_district_d_ytd, key, row, wh_to_part(wid));
	}
}
