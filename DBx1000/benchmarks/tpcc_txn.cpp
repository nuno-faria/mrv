...
RC tpcc_txn_man::run_txn(base_query * query) {
	...
	if (MODE == TPC_C) {
		...
		if (MODE == READ) {
			return run_simple_read(m_query);
		}
		else {
			return run_simple_update(m_query);
		}
	}
}


RC tpcc_txn_man::run_simple_update(tpcc_query *query) {
	uint64_t w_id = query->w_id;
	int sign = rand() % 2 == 0 ? -1 : 1;
	double amount = sign * (1 + rand() % 5);

	if (TYPE == NATIVE) {
		INDEX *index = _wl->i_warehouse;
		itemid_t *item = index_read(index, w_id, wh_to_part(w_id));
		assert(item != NULL);
		row_t *r_wh = ((row_t *)item->location);
		row_t *r_wh_local = get_row(r_wh, WR);
		...
		if (w_ytd + amount >= 0) {
			r_wh_local->set_value(W_YTD, w_ytd + amount);
		}
		else {
			return finish(Abort);
		}
		...
		INDEX *index_mrv = _wl->i_warehouse_w_ytd;
		bool done = false;
		int rk = URand(0, MRV_SIZE - 1, 0);
		int rk0 = rk;

		while (!done) {
			uint64_t mrv_key = (w_id << 8) | rk;
			itemid_t *item_mrv = index_read(index_mrv, mrv_key, wh_to_part(w_id));
			assert(item_mrv != NULL);
			row_t *r_mrv = get_row((row_t *)item_mrv->location, WR);
			...
			double partial_w_ytd;
			r_mrv->get_value(1, partial_w_ytd);

			// add
			if (amount >= 0) {
				r_mrv->set_value(1, partial_w_ytd + amount);
				done = true;
			}
			// sub
			else {
				double max_decrement = min(partial_w_ytd, -amount);
				r_mrv->set_value(1, partial_w_ytd - max_decrement);
				amount += max_decrement;
				if (amount >= 0) {
					done = true;
				}
				else {
					rk = (rk + 1) % MRV_SIZE;
					// iterated over all records
					if (rk == rk0) {
						return finish(Abort);
					}
				}
		...
		assert(item != NULL);
		row_t *r_wh = ((row_t *)item->location);
		int sign = URand(0, 1, 0) == 0 ? -1 : 1;
		double amount = sign * URand(1, 5, 0);

		row_t *r_wh_local = get_row(r_wh, WRESC, W_YTD, amount);
...
RC tpcc_txn_man::run_simple_read(tpcc_query *query) {
	uint64_t w_id = query->w_id;

	if (TYPE == NATIVE || TYPE == ESCROW) {
		INDEX *index = _wl->i_warehouse;
		itemid_t *item = index_read(index, w_id, wh_to_part(w_id));
		assert(item != NULL);
		row_t *r_wh = ((row_t *)item->location);
		...
		for (int rk = 0; rk < MRV_SIZE; rk++) {
			uint64_t mrv_key = (w_id << 8) | rk;
			itemid_t *item_mrv = index_read(index_mrv, mrv_key, wh_to_part(w_id));
			assert(item_mrv != NULL);
			row_t *r_mrv = get_row((row_t *)item_mrv->location, RD);
...
//TODO generic mrv update instead of copy and paste
RC tpcc_txn_man::run_payment(tpcc_query * query) {
	...
	if (g_wh_update) {
		...
		// ESCROW
			...
			r_wh_local = get_row(r_wh, WRESC, W_YTD, query->h_amount);
			if (r_wh_local == NULL) {
				return finish(Abort);
			}
		}
	...
	// MRV
		...
		bool done = false;
		while (!done) {
			uint64_t mrv_key = query->w_id << 16 | query->d_id << 8 | rk;
			INDEX *index_mrv = _wl->i_district_d_ytd;
			itemid_t *item_mrv = index_read(index_mrv, mrv_key, wh_to_part(query->w_id));
			assert(item_mrv != NULL);
			row_t *r_mrv = get_row((row_t *)item_mrv->location, WR);
			...
			double partial_d_ytd;
			r_mrv->get_value(1, partial_d_ytd);

			// add
			if (amount >= 0) {
				r_mrv->set_value(1, partial_d_ytd + query->h_amount);
				done = true;
			}
			// sub
			else {
				double max_decrement = min(partial_d_ytd, -amount);
				r_mrv->set_value(1, partial_d_ytd - max_decrement);
				amount += max_decrement;
				if (amount >= 0) {
					done = true;
				}
	...
	// ESCROW
		...
		r_dist_local = get_row(r_dist, WRESC, D_YTD, query->h_amount);
