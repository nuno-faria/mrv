...
void txn_man::cleanup(RC rc) {
	...
	for (int rid = row_cnt - 1; rid >= 0; rid --) {
		...
		if (type != WRESC) {
			...
			orig_r->return_row_escrow(type, this, accesses[rid], rc);
		}
		#endif
...
#if TYPE == ESCROW
row_t *txn_man::get_row(row_t *row, access_t type, int escrow_column, int escrow_delta) {
	assert(type == WRESC);
	...
	accesses[row_cnt]->escrow_column = escrow_column;
	accesses[row_cnt]->escrow_delta = escrow_delta;

	rc = row->get_row(type, this, accesses[row_cnt]->data, escrow_column, escrow_delta);
