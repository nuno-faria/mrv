...
#if TYPE == ESCROW
RC row_t::get_row(access_t type, txn_man *txn, row_t *&row, int escrow_column, double escrow_delta) {
	assert(type == WRESC);
	
	RC rc = RCOK;
	lock_t lt = LOCK_SH;
	rc = this->manager->lock_get_escrow(lt, txn, escrow_column, escrow_delta);
...
#if TYPE == ESCROW
void row_t::return_row_escrow(access_t type, txn_man *txn, Access* access, RC rc) {
	assert(type == WRESC);
	if (rc == RCOK) {
		this->copy(access->data);
	}
	else {
		this->copy(access->orig_data);
	}
	this->manager->lock_release_escrow(txn, this, access, rc);
}
#endif