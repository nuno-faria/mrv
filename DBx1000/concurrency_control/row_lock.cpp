...
RC Row_lock::lock_get_escrow(lock_t type, txn_man *txn, int escrow_column, double escrow_delta) {
	uint64_t *txnids = NULL;
	int txncnt = 0;
	return lock_get_escrow(type, txn, txnids, txncnt, escrow_column, escrow_delta);
}


RC Row_lock::lock_get_escrow(lock_t type, txn_man *txn, uint64_t *&txnids, int &txncnt, int escrow_column, double escrow_delta) {
	RC rc = lock_get(LOCK_SH, txn, txnids, txncnt);
	...
	if (rc == RCOK) {
		if (escrow_delta < 0) {
			...
			this->_row->get_value(escrow_column, quantity_on_hand);

			// check cond
			if (quantity_on_hand + this->negative_escrows[escrow_column] + escrow_delta >= 0) {
				this->negative_escrows[escrow_column] += escrow_delta;
			}
			else {
				rc = Abort;
			}
...
RC Row_lock::lock_release_escrow(txn_man *txn, row_t* row, Access* access, RC rc) {
	assert(access->type == WRESC);
	...
	if (rc == RCOK) {
		double qoh;
		row->get_value(access->escrow_column, qoh);
		assert(qoh + access->escrow_delta >= 0);
		row->set_value(access->escrow_column, qoh + access->escrow_delta);
	}

	// release escrow
	if (access->escrow_delta < 0) {
		this->negative_escrows[access->escrow_column] -= access->escrow_delta;
	}
